Feature:Azure Data Ingest
  ...
  Scenario Outline: Data Factory Ingest SQL Database into ADLS
    Given the ADF pipeline ingest_azure_sql_andytest has been triggered with <parameters>
    And I poll the pipeline every 10 seconds until it has completed
    And the ADF pipeline ingest_azure_sql_andytest has finished with state Succeeded
    And the ADF pipeline completed in less than 480 seconds
    Then the files <output_files> are present in the ADLS container raw in the directory ingest_azure_sql_andytest
